...
let check_monop w t =
  match w with
      Uminus ->
	if not (same_type t Types.integer) then expr_fail ();
	Types.integer
    | Not ->
	if not (same_type t Types.boolean) then expr_fail ();
	Types.boolean
...
let check_binop w t1 t2 =
  match w with
	...
	if not (same_type t1 Types.integer && same_type t2 Types.integer) then
	  expr_fail ();
	Types.integer
	...
	Types.boolean
    | And | Or ->
        if not (same_type t1 Types.boolean && same_type t2 Types.boolean) then
	  expr_fail ();
	Types.boolean
...
and expr_type e env =
  match e.e_guts with
    ...
    | Sub (e1, e2) ->
        ...
        if not (same_type t2 Types.integer) then
    ...
    | FuncCall (p, args) -> 
        ...
        if same_type t1 Types.voidtype then
...
and check_libcall q args env v =
  ...
  match q.q_id with 
	...
	if not (same_type t1 Types.integer) || not (is_string t2) then
...
let rec check_stmt s env alloc =
  ...
  match s.s_guts with
	...
	begin
	  match res with
	      Some e ->
	  	if same_type !return_type Types.voidtype then
    ...
    | ForStmt (var, lo, hi, body, upb) ->
        ...
        if not (same_type vt Types.integer) || not (same_type lot Types.integer)
            || not (same_type hit Types.integer) then
        ...
        let d = make_def (intern "*upb*") VarDef Types.integer in
        alloc d; upb := Some d
...
let local_alloc size nreg d =
  if !regvars && not d.d_mem && scalar (d.d_type) 
      && !nreg < Metrics.nregvars then begin
...
let rec check_typexpr te env = 
  match te with
	...
	let size = ref 0 in
        ...
        align Metrics.max_align size;
	let r = { r_size = !size; r_align = Metrics.max_align } in
	...
	let t = 
 	...
 	mk_type (PointerType t) Metrics.addr_rep
...
and check_decl d env = 
  ...
  match d with
	...
	let add_hole (x, te, h) env1 =
	  add_def (make_def x (HoleDef h) Types.voidtype) env1 in
...
and check_heading env (Heading (x, fparams, result)) =
  ...
  let rt = (match result with
	      Some te -> check_typexpr te env | None -> Types.voidtype) in
  if not (same_type rt Types.voidtype) && not (scalar rt) then
    sem_error "return type must be scalar" [];
  let p = { p_fparams = defs; p_pcount = !pcount; p_result = rt } in
  mk_type (ProcType p) Metrics.proc_rep
