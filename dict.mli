...
and ptype = 
  { t_id: int;			(* Unique identifier *)
    ...
    t_rep: Target.metrics }	(* Runtime representation *)
...
val mk_type : type_guts -> Target.metrics -> ptype
...
(* All the types and functions above are machine-independent, but the
   basic types listed below have sizes and alignments that are dependant
   on the target. *)

module type TypesT = sig
 module Metrics : Target.MetricsT
 val integer : ptype
 val character : ptype
 val boolean : ptype
 val voidtype : ptype
...
module TypesF(Metrics : Target.MetricsT): TypesT
